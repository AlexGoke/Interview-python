...
（最大堆堆顶就是最接近并且<=中位数；最小堆堆顶就是最接近并且>=中位数）

当目前两堆总数为偶数的时候，把数字存入最大堆，然后重排最大堆，如果最大堆的堆顶数字大于最小堆堆顶数字，则把两个堆顶数字交换，重排两堆，此时两堆数字总数为奇数，直接输出最大堆堆顶数字即为中位数；

如果当前两堆总数为奇数的时候，把数字存入最小堆，重排最小堆，如果最大堆的堆顶数字大于最小堆堆顶数字，则把两个堆顶数字交换，重排两堆，此时两堆数字总数为偶数，取两堆堆顶数字做平均即为中位数。
'''
...
class Solution:
    ...
    #调用该函数时，刚刚加入一个数字后，重拍，把新的最大值找出来放在堆顶
    def MaxHeap(self, alist):
        ...
        for i in range(length//2-1, -1, -1):
            ...
            while not heap and 2*k < length-1:
                ##index应该是传入数列的最右端的值
                index = 2*k+1    #当length为偶数时，index=2*k+1 是最右端的下标(length-1)
                if index < length - 1:    #当length为奇数时，index+1 才是最右端的下标
